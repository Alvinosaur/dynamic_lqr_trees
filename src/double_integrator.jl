...
@inline function cost_optimal(x0::Vec2f, v0::Vec2f, x1::Vec2f, v1::Vec2f)
    ...
    q = 24*dot(v0+v1, x01)
    r = -36*dot(x01, x01)
    cost(t) = t + dot(v01, 4.0*v01/t -6(-v0*t+x01)/t^2) + (
        dot(-6*v01/t^2 + 12(x01-v0*t)/t^3, -v0*t+x01))
...
function filter_reachable(Sset::Vector{Vec4f}, idxset::Vector{Int64}, 
    ...
    for idx in idxset
        @inbounds s = Sset[idx]
        if IsReachable(s)
            # if backwards, cost from s -> s_c
            # if forwards,  cost from s_c -> s

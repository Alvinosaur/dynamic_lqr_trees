...
@inline function isValid(this::World, state)
    # check if the sampled point is inside the world"
    all(this.lower_bounds .< state) || return false
    all(state .< this.upper_bounds) || return false

    # check if point is not already sampled (within some radius of another point)
    for P in this.Pset
        isInside(P, Vec3f(state[1:3])) && return false
    end
    return true
end

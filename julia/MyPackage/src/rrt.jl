...
function BuildPath(this::RRT)
    ...
    while length(open) > 0
        ...
        for (trans_cost, next_idx, _, _) in neighbors
            ...
            if get(G, next_idx, nothing) == nothing || new_g < G[next_idx]
                G[next_idx] = new_g
                traj, U = GenerateTrajectory(this, this.start, this.vertices[next_idx])
                h = Distance(this, traj)
                f = new_g + ϵ * h
                push!(open, (f, next_idx))
                successor[next_idx] = (cur_idx, U)
            end
        end
    end
    ...
    while cur_idx != goal_idx
        cur_idx, U = successor[cur_idx]
        push!(path, cur_idx)
        push!(ctrl_traj, U)
    end
    return path, ctrl_traj
end
